...
try {
  const r = (typeof routeOnly === 'function') ? routeOnly() : (location.hash||'').replace(/^#\/?/, '').split('?')[0] || 'home';
  if (r === 'store') { ensureStoreCartSidebar(); renderStoreCartSidebar(); }
} catch(_) {}
...
// Guard: suppress clicks right after horizontal drag
try {
...
function ensureStoreCartSidebar(){
  if (typeof routeOnly !== 'function' || routeOnly() !== 'store') return;
  ...
  right.innerHTML = `
    <div class="cart-side__head"><h3>장바구니</h3></div>
...
function attachHeaderHeightWatchers(){
  try {
    ...
    let _t; const onResizeOrScroll = () => { cancelAnimationFrame(_t); _t = requestAnimationFrame(updateHeaderHeightVar); };
