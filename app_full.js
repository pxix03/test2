...
function patchLegacyLinks() {
  ...
  // <form action="*.html"> → SPA에서 라우트만 이동
...

 {
  ...
  if (card && !isInteractive(e.target)) {
    ...
    if (firstLinkEl) {
      ...
      const to = firstLinkEl.getAttribute('data-link') || href.replace(/^#\//,'');
